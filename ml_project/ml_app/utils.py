...
# stream = True will not save the video, as a result i can't save in database.
...
def upload_image_video(dir_name):
    if str(dir_name) == "images":
        ...
        newest_user_image = max(
            [
                f
                for f in os.listdir(user_file_path)
                if os.path.isfile(os.path.join(user_file_path, f))
            ],
            key=lambda x: os.path.getctime(os.path.join(user_file_path, x)),
        )
        return newest_user_image

    if str(dir_name) == "videos":
        user_video_path = os.path.join("media", "videos")
        newest_user_video = max(
            [
                f
                for f in os.listdir(user_video_path)
                if os.path.isfile(os.path.join(user_video_path, f))
            ],
            key=lambda x: os.path.getctime(os.path.join(user_video_path, x)),
        )
...
def image_video_path():
    ...
    newest_file = max(
        [
            f
            for f in os.listdir(newest_directory_path)
            if os.path.isfile(os.path.join(newest_directory_path, f))
        ],
        key=lambda x: os.path.getctime(os.path.join(newest_directory_path, x)),
    )
